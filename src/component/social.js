...
export class Social extends abstractComponent {
  getSocial () {
    return {
      'github': {
        url: '#',
  ...
  render () {
    return `<ul class="social">
           ${Object.keys(this.getSocial()).map((social) =>`
              <li><a href="${this.getSocial()[social].url}" title="${social}" preserveaspectratio="xMidYMid meet">
                  <svg viewBox="0 0 512 512">
                     <path d="${this.getSocial()[social].path}"></path>
              </svg></a></li>
           `).join('')}
          </ul>`
  }
